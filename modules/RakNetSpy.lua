...
local function blockPacket(packet)
	local read, blockMethod = pcall(function()
		return packet.Block
	end)

	if not read then
		return false, safeError(blockMethod)
	elseif type(blockMethod) ~= "function" then
		return false, "RakNetPacket:Block is unavailable"
	end

	local ran, blockError = pcall(blockMethod, packet)

	if not ran then
		return false, safeError(blockError)
	end
...
local function capture(packet, direction)
	...
	local blockRecord = packetIdRead and blockedPacketIds[packetIdKey(packetId)] or nil
	local blocked = false
	local blockError

	if blockRecord then
		blocked, blockError = blockPacket(packet)

		if blocked then
			diagnostics.PacketsBlocked = diagnostics.PacketsBlocked + 1
		else
			diagnostics.BlockErrors = diagnostics.BlockErrors + 1
			diagnostics.LastBlockError = {
				Direction = direction,
				PacketId = packetId,
				Error = blockError,
			}
	...
	entry.BlockConfigured = blockRecord ~= nil
	entry.Blocked = blocked
	entry.BlockError = blockError
...
local function buildReceiveTemplate(entry)
	...
	local idDeclaration = packetId and ("local TARGET_PACKET_ID = %s\n\n"):format(packetId) or ""
	local idCondition = packetId and "\tif packet.PacketId ~= TARGET_PACKET_ID then\n\t\treturn\n\tend\n\n" or ""
	...
	return table.concat({
		...
		idDeclaration,
		"local function onPacketReceived(packet)\n",
		idCondition,
		"\t-- Inspect packet.AsBuffer, call packet:SetData(...), or packet:Block().\n",
		'\tprint("RakNet receive", packet.PacketId, packet.Size)\n',
		"end\n\n",
		"raknet.add_receive_hook(onPacketReceived)\n",
		"-- Remove later with: raknet.remove_receive_hook(onPacketReceived)\n",
		"return onPacketReceived",
	})
end

local function validateScript(source)
	if type(validateGeneratedSource) == "function" then
		local ran, valid, validationError = pcall(validateGeneratedSource, source, "@HydroxideRakNetCode")

		if not ran then
			return false, safeError(valid)
		end

		return valid, validationError
	elseif type(loadstring) == "function" then
		local ran, chunk, compileError = pcall(loadstring, source, "@HydroxideRakNetCode")

		if not ran then
			return false, safeError(chunk)
		elseif type(chunk) ~= "function" then
			return false, safeError(compileError or "the compiler returned no function")
		end
	end
...
local function finishScript(source)
	if #source > maxGeneratedOutputBytes then
		return nil, ("Generated RakNet script would exceed the %d-byte output limit"):format(maxGeneratedOutputBytes)
	end

	local valid, validationError = validateScript(source)

	if not valid then
		return nil, "Generated RakNet source did not compile: " .. safeError(validationError)
	end
...
local function buildScript(entry)
	...
	elseif entry.Direction == "receive" then
		return finishScript(buildReceiveTemplate(entry))
	...
	local prefix = "-- Generated by Hydroxide RakNet Spy\n"
		.. "-- Outgoing packet replay with the exact captured payload.\n"
		.. "local payload = buffer.fromstring("
	local suffix = (")\n\nreturn raknet.send(payload, %s, %s, %s)"):format(priority, reliability, orderingChannel)
	...
	return finishScript(prefix .. quoteBytes(entry.PayloadBytes) .. suffix)
end
...
local function replay(entry)
	...
	elseif blockedPacketIds[packetIdKey(entry.PacketId)] then
		diagnostics.ReplayFailures = diagnostics.ReplayFailures + 1
		return false, "This RakNet packet ID is blocked"
	elseif
...
RakNetSpy.BlockedPacketIds = blockedPacketIds
RakNetSpy.Retention = {
	MaxLogs = maxLogs,
...
RakNetSpy.IsPacketIdBlocked = function(first, second)
	local packetId = first == RakNetSpy and second or first
	return blockedPacketIds[packetIdKey(packetId)] ~= nil
end
RakNetSpy.SetPacketIdBlocked = function(first, second, third)
	local packetId = first == RakNetSpy and second or first
	local blocked = first == RakNetSpy and third or second
	local key = packetIdKey(packetId)
	local existing = blockedPacketIds[key]
	...
	if blocked == false then
		...
		blockedPacketIds[key] = nil
		emit(existing, "unblocked")
		return false
	elseif existing then
		return true
	end
	...
	blockedPacketIds[key] = record
	emit(record, "blocked")
	return true
end
RakNetSpy.BlockPacketId = function(first, second)
	local packetId = first == RakNetSpy and second or first
	return RakNetSpy.SetPacketIdBlocked(packetId, true)
end
RakNetSpy.UnblockPacketId = function(first, second)
	local packetId = first == RakNetSpy and second or first
	return RakNetSpy.SetPacketIdBlocked(packetId, false)
end
RakNetSpy.ClearBlockedPacketIds = function()
	local changed = next(blockedPacketIds) ~= nil

	for key in pairs(blockedPacketIds) do
		blockedPacketIds[key] = nil
	end

	if changed then
		emit(nil, "blocked-cleared")
	end

	return changed
end
RakNetSpy.ClearPacketFilters = function()
	local ignoredChanged = next(ignoredPacketIds) ~= nil
	local blockedChanged = next(blockedPacketIds) ~= nil
	...
	if ignoredChanged or blockedChanged then
		emit(nil, "filters-cleared")
	end

	return ignoredChanged or blockedChanged
end
